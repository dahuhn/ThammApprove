...
// Endgültige Version mit 100% korrekter Switch API
...
async function jobArrived(s, job) {
    ...
    try {
        ...
        // Properties korrekt mit job-Parameter holen
        const apiUrl = s.getPropertyValue("apiUrl", job) || "http://172.16.0.66:3101";
        await job.log(LogLevel.Info, `${scriptName}: Using API URL: ${apiUrl}`);

        const customerEmailFromProps = s.getPropertyValue("customerEmail", job);
        const customerNameFromProps = s.getPropertyValue("customerName", job);
        ...
        // Get job information mit korrekter API
        const jobPath = await job.get(AccessLevel.ReadOnly);
        const fileName = job.getName(); // Synchron
        ...
        if (result.success) {
            ...
            // Success - Route to Pending Folder
